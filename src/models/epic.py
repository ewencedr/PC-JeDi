...
class Hybrid_EPiC_layer_cond_mask(nn.Module):
    def __init__(self, local_in_dim, hid_dim, latent_dim, cond_feats=1, time_cond=1, sum_scale=1e-2):
        super().__init__()
        ...
        fc_global2 = nn.Linear(hid_dim + cond_feats, latent_dim)
        self.fc_global2 = weight_norm(fc_global2, ["weight"])
        fc_local1 = nn.Linear(local_in_dim + latent_dim + time_cond, hid_dim)
        self.fc_local1 = weight_norm(fc_local1, ["weight"])
        fc_local2 = nn.Linear(hid_dim + time_cond, hid_dim)
        self.fc_local2 = weight_norm(fc_local2, ["weight"])

        self.sum_scale = sum_scale

    def forward(self, x_global, x_local, cond_tensor, time_cond, mask):  # shapes:
        batch_size, n_points, latent_local = x_local.size()
        latent_global = x_global.size(1)
        ...
        # first add dimension, than expand it
        x_global2local = x_global.view(-1, 1, latent_global).repeat(1, n_points, 1)
        x_time = time_cond.view(-1, 1, time_cond.shape[-1]).repeat(1, n_points, 1)
        x_localCATglobal = torch.cat([x_local, x_global2local, x_time], 2)
        # with residual connection before AF
        x_local1 = F.leaky_relu(self.fc_local1(x_localCATglobal))
        x_local1 = torch.cat([x_local1, x_time], axis=2)
        x_local = F.leaky_relu(self.fc_local2(x_local1) + x_local)

        return x_global, x_local


class Hybrid_EPiC_Encoder(nn.Module):
    def __init__(
        self,
        hid_d: int,
        feats: int,
        ...
        fc_g1 = nn.Linear(int(2 * self.hid_d), self.hid_d)
        self.fc_g1 = weight_norm(fc_g1, ["weight"])
    ...
    def forward(self, x, mask, cond_tensor, time_tensor):
        n_points = x.shape[1]
        x_time = time_tensor.view(-1, 1, cond_tensor.shape[-1]).repeat(1, n_points, 1)
        x_local_cat = torch.cat([x, x_time], 2)
        x_local = F.leaky_relu(self.fc_l1(x_local_cat))
        x_local_cat = torch.cat([x_local, x_time], 2)
        x_local = F.leaky_relu(self.fc_l2(x_local_cat) + x_local)
        ...
        for i in range(self.equiv_layers):
            # contains residual connection
            x_global, x_local = self.nn_list[i](x_global, x_local, cond_tensor, time_tensor, mask)
        return self.output_dense(x_local)