...
# epic jedi 30
epic_jedi_30_cond = ABS_PATH / "epic_jedi1/2023-06-06_15-48-58-013386"
# epic_jedi_30_uncond = ABS_PATH / "epic_uncond_30/2023-05-19_09-45-57-057758/"

# epic jedi 150
epic_jedi_150_cond = ABS_PATH / "epic_jedi1/2023-06-06_15-48-57-904902/"
...
def get_scores(path, sampler_list=["ddim_200", "em_200"]):
    ...
    for sampler in sampler_list:
        # Read the yaml file
        yaml_file = path / f"outputs/t/yaml_scores/{sampler}"
...
def shape_df(df):
    ...
    transposed_df["fpnd"] = (
        "-" if (transposed_df["fpnd"] == "-999.0000").any() else transposed_df["fpnd"]
    )

    transposed_df["W1EFP"] = (
        transposed_df["w1efp"].astype(str)
        + " +- "
        + transposed_df["w1efp_err"].astype(str)
    )
    transposed_df["W1M"] = (
        transposed_df["w1m"].astype(str) + " +- " + transposed_df["w1m_err"].astype(str)
    )
    transposed_df["W1P"] = (
        transposed_df["w1p"].astype(str) + " +- " + transposed_df["w1p_err"].astype(str)
    )

    transposed_df = transposed_df.drop(
        columns=["w1efp", "w1efp_err", "w1m", "w1m_err", "w1p", "w1p_err"]
    )
    return transposed_df


epic_jedi_150_cond_scores = get_scores(
    epic_jedi_150_cond, sampler_list=["ddim_200", "em_200"]
)
...
epic_jedi_30_cond_scores = get_scores(
    epic_jedi_30_cond, sampler_list=["ddim_200", "em_200"]
)
...
# epic_model_150_scores = pd.concat([epic_jedi_150_cond_scores], axis=0, keys=["Conditional", "Unconditional"])
# pc_model_150_scores = pd.concat([pc_jedi_150_cond_scores,pc_jedi_150_uncond_scores], axis=0, keys=["Conditional", "Unconditional"])

# model_150_scores = pd.concat([epic_model_150_scores], axis=0, keys=["EPIC", "PC"])

print(epic_jedi_30_cond_scores.style.to_latex())

print("===================")
print(epic_jedi_150_cond_scores.style.to_latex())
print(epic_jedi_150_cond_scores)
